...
```
```{r Data Plotting, include=TRUE}
plot_ly(fires_daily, x = ~Date, y = ~DNRF) %>%
  add_lines(line = list(color = "#E44D2E", width = 2), name = "Fires") %>%
  add_markers(marker = list(color = "#A30000", size = 4), showlegend = FALSE) %>%
  layout(
    title = list(text = "Daily Wildfire Frequency (2024)", x = 0),
    xaxis = list(title = "Month", showgrid = FALSE),
    yaxis = list(title = "Number of Recorded Fires"),
    hovermode = "x unified",
...
##Daily Wildfire Frequency



```{r Data Plotting2, include=TRUE}
renderPlotly({
  plot_ly(fires_daily, x = ~Date, y = ~DNRF) %>%
    add_lines(line = list(color = "#E44D2E", width = 2), name = "Fires") %>%
    add_markers(marker = list(color = "#A30000", size = 4), showlegend = FALSE) %>%
